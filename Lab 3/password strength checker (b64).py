...
# With an added functionality of storing all the tested passwords in an encrypted base64 format locally
...
try:
    ...
    # stores all the tested passwords in a text file withbase64 encoding
    def encode_password(password, filename = "password history encoded (base64).txt") :

        # converts the password to a bytes object
        # converts that to a base64-encoded version bytes object
        ...
        with open(filename, "a") as file :
            file.write(base64_password + "\n")

    # This method will decrypt all the passwords inside the text file and store them in an array
    def get_encoded_password_history(filename="password history encoded (base64).txt") :
        try:

            with open(filename, "r") as file :
                
                encode_passwords = [line.strip() for line in file.readlines()]
                return [base64.b64decode(ep.encode()).decode() for ep in encode_passwords]
    ...
    def main () :
        ...
        while(True):
            ...
            if passwrd_len != (up + low + digit + special) :
                ...
                print(get_encoded_password_history)
            ...
            if category == 'Medium' :
                ...
                print("Password History: ")
                encode_password(password)
                history = get_encoded_password_history()
                print(history)
